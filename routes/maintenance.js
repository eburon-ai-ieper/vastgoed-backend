...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    await createNotification(
      ...
      `New ${category} request from renter: ${title}`,
      requestId
    );
    ...
    res.status(201).json({ 
      id: requestId, 
      message: 'Maintenance request created. Broker has been notified.',
      status: 'pending'
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
router.post('/:id/select-contractor', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({ message: 'Contractor selected. Contractor and broker have been notified.' });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
